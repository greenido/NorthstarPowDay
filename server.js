...
app.get("/snow/", function (request, response) {
  ...
  rp("https://www.onthesnow.com/california/northstar-california/skireport.html")
    .then(function (body) {
      // Process html
      let html = body;
      let inx1 = html.indexOf("Last Updated:") + 11;
      let inx2 = html.indexOf(">", inx1) + 10; // we got 2 > to skip
      let inx3 = html.indexOf("</time", inx2);
      let lastUpdate = html.substring(inx2, inx3).trim();
      let lastUpdateStr = "";
      if (lastUpdate != null && lastUpdate.length > 2) {
        lastUpdateStr = "<br><small>🏄🏽‍♀️ " + lastUpdate + " 🏂</small>";
      }
      //console.log(`inx1: ${inx1} inx2: ${inx2} inx3: ${inx3}`);
      console.log("== lastUpdate: " + lastUpdate);
      let res = "";
      res += lastUpdateStr;
      inx1 = html.indexOf("styles_snow__3fbcm") + 20;
      inx2 = inx1 + 6;
      for (let i = 0; i < 18; i++) {
        inx2 = html.indexOf("</div>", inx2 + 5);
      }

      let allDataDiv = html.substring(inx1, inx2);
      //console.log(`== inx1: ${inx1} inx2: ${inx2}  dataDiv: ${allDataDiv}`);
      res += allDataDiv;
      response.send(res);
    })
    .catch(function (err) {
    ...
    request(
      ...
      function (err, response, body) {
        if (err) {
          ...
          assistant.tell(
            "Sorry something is not working at the moment. Please try again later and be happy."
          );
          return;
        }
        try {
          ...
          if (snowToday == null || snowToday.length < 1) {
            assistant.ask(
              "Could not find if there is powder today. See you later!"
            );
            return;
          }

          let res =
            "Today at Northstar we got " +
            snowToday +
            " inch of snow. In the upper mountain we have " +
            upperSnow +
            " inch and in the lower moutain there are " +
            lowerSnow +
            " inch. Have an amazing day and be safe!";
          assistant.tell(res);
        } catch (error) {
          console.log(
            "(!) Error: " + error + " json: " + JSON.stringify(error)
          );
        }
      }
    ); //
...
app.use(function (err, req, res, next) {
  ...
  res
    .status(500)
    .send("Oppss... could not check when is the next flight to space.");
});
